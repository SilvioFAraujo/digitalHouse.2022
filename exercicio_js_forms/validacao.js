...
formulario.onsubmit = function(evento){
    ...
    if(campoNome.value.length < 2 || campoNome.value.length > 80){
        erroEncontrado = true;
        var erroNome = campoNome.parentElement.querySelector(".invalid-feedback");
        erroNome.innerText = "O nome do usuário é obrigatório";
    }
    if(campoSobrenome.value.length < 2 || campoSobrenome.value.length > 100){
        erroEncontrado = true;
    ...
    if(campoSenha.value.length > 8 || campoSenha.value.length > 100){
        erroEncontrado = true;
    }
    if(campoUsername.value.length > 10 || campoUsername.value.length > 15){
        erroEncontrado = true;
    }
